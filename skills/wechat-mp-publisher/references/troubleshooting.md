...
- 本地 Hermes profile 只负责发指令给 MCP，不直接与微信交互。
...
**常见原因**:
1. **相对路径失效**: 远程 MCP 通常无法访问你本地磁盘上的相对路径 `./assets/cover.jpg`。
2. **本机绝对路径同样不可见**: 即使写成 `/root/.../cover.png`，远程服务大多也读不到。
3. **图床防盗链**: 使用了某些开启防盗链的图床链接。
...
- 先优先确认当前默认主题 `sspai` 是否可用；不要再用旧主题示例或临时主题注册路径混入生产链路。
