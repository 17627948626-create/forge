...
These templates define the structural skeleton for each article type.

## Table of Contents

- [How Templates Are Used](#how-templates-are-used)
- [Template 1: 资讯 (News)](#template-1-资讯-news--updates)
- [Template 2: 周报 (Weekly Roundup)](#template-2-周报-weekly-roundup)
- [Template 3: 教程 (Tutorial)](#template-3-教程-tutorial--how-to)
- [Template 4: 观点 (Opinion)](#template-4-观点-opinion--analysis)
- [Template 5: 科普 (Explainer)](#template-5-科普-explainer--education)
...
> ⚠️ **草稿 frontmatter ≠ 发布 frontmatter**
> 本文件中的 frontmatter 示例（含 `date`、`tags`、`type`、`issue_number` 等字段）用于写作阶段的草稿文件（`draft*.md`、`final*.md`），是内容结构元数据，**不是** 最终发布用的 `publish.md` 格式。
> 最终 `publish.md` 必须使用 SKILL.md Key Rules 章节规定的 6 个标准字段（`title`/`author`/`profile`/`cover`/`theme`/`slug`），任何其他字段均不允许出现在 `publish.md` 中。
